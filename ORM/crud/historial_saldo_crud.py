...
class HistorialSaldoCRUD:
    ...
    def obtener_por_id(db: Session, movimiento_id: int) -> Optional[HistorialSaldo]:
        return db.query(HistorialSaldo).filter(HistorialSaldo.id == movimiento_id).first()

    @staticmethod
    def obtener_todos(db: Session, skip: int = 0, limit: int = 100) -> List[HistorialSaldo]:
        return db.query(HistorialSaldo).offset(skip).limit(limit).all()

    @staticmethod
    def eliminar_movimiento(db: Session, movimiento_id: int) -> bool:
        movimiento = db.query(HistorialSaldo).filter(HistorialSaldo.id == movimiento_id).first()
        if not movimiento:
            return False
