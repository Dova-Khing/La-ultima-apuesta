...
def registrar_usuario() -> Usuario:  # retorna un objeto Usuario
    ...
    saldo_inicial: float = (
        Validaciones.validar_saldo()
    )  # llama la validacion de el saldo inicial
    # Se construye instancia de Usuario
    jugador: Usuario = Usuario(
        nombre, edad, saldo_inicial
    )  # instancia de jugador donde se tiene las variables que lo conforman
    return jugador
...
=============================================
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print(
                f" ! Hasta Pronto !, Gracias por jugar {jugador.nombre} "
            )  # Mensaje de despedida con el nombre del jugador
            break
